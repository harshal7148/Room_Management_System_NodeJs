...
router.get('/getOutstandingHistory/:ownerId', (req, res) => {
    outstandingHistory.find({}).
        populate('tenantId', select = ['name', 'isActive', 'roomNo', 'depositeAmount', 'profilePic'])
                ...
                then(oustandingRes => {
                    if (stringUtil.isNullorEmpty(oustandingRes)) {
                        ...
                        outstandingHistory.find({ tenantId: response._id }).then((response) => {
                            const result = dateUtil.calcNextMonthDate(response[0].histories[response[0].histories.length - 1].toDate)
                            if (result.isValid) {
